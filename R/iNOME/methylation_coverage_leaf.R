...
leaf_coverage_GC <- list()
leaf_coverage_non_GC <- list()
for(i in files){
  ...
  leaf_coverage_GC[[i]] <- list()
  leaf_coverage_non_GC[[i]] <- list()
  for(j in 1:3){
    leaf_coverage_non_GC[[i]][[j]] <- temp_CX %>%
    ...
    leaf_coverage_GC[[i]][[j]] <- temp_CX %>%
      filter(context == contexts[j],sequence == "GC") %>%
      computeMethylationDataCoverage(methylationData = .,context = contexts[j],breaks = seq(1,15,1)) %>%
      tibble(position = seq(1,15,1),coverage = .,context = paste0("G",contexts[j]),sample = gsub("_CX_report.txt","",i))
  }
  leaf_coverage_GC[[i]] <- bind_rows(leaf_coverage_GC[[i]])
  leaf_coverage_non_GC[[i]] <- bind_rows(leaf_coverage_non_GC[[i]])
}
leaf_coverage_non_GC <- bind_rows(leaf_coverage_non_GC)
leaf_coverage_GC <- bind_rows(leaf_coverage_GC)
# Plotting methylation coverage
colours <- c('#8b0000', '#a56252', '#a9a9a9', '#8e65b1', '#00008b')
leaf_coverage_GC <- leaf_coverage_GC %>%
  mutate(time = gsub(".*_","",sample),sample = gsub("_.*","",sample))
leaf_coverage_GC$time <- factor(leaf_coverage_GC$time,levels = c("6h","12h","24h"))

leaf_coverage_non_GC <- leaf_coverage_non_GC %>%
  mutate(time = gsub(".*_","",sample),sample = gsub("_.*","",sample))
leaf_coverage_non_GC$time <- factor(leaf_coverage_non_GC$time,levels = c("6h","12h","24h"))

ggplot(leaf_coverage_GC,aes(x = position,y = coverage,colour = sample)) +
  geom_line() +
...
ggsave("~/iNOME/Figures/Supplementary/leaf_coverage_GC_Col0.pdf")

colours <- c('#8b0000', '#a56252', '#a9a9a9', '#8e65b1', '#00008b')
leaf_coverage_non_GC %>%
  ...
  theme_cowplot() +
  theme(strip.background = element_blank(),axis.text = element_text(size = 8))
ggsave("~/iNOME/Figures/Supplementary/leaf_coverage_non_GC_Col0.pdf")
