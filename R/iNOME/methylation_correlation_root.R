...
#### Methylation correlation for root ####
## Loading Fasta file
...
root_correlation_GC <- list()
root_correlation_non_GC <- list()
for(i in files){
  ...
  root_correlation_GC[[i]] <- list()
  root_correlation_non_GC[[i]] <- list()
  for(j in 1:3){
    root_correlation_GC[[i]][[j]] <- temp_GC %>%
    ...
    root_correlation_non_GC[[i]][[j]] <- temp_CX %>%
      computeMethylationDataSpatialCorrelation(methylationData = .,context = contexts[j],distances = c(50,100,200,500,1000,2000,5000)) %>%
      tibble(position = seq(1,7,1),correlation = .,context = paste0("G",contexts[j]),sample = gsub("_CX_report.txt","",i))
  }
  root_correlation_GC[[i]] <- bind_rows(root_correlation_GC[[i]])
  root_correlation_non_GC[[i]] <- bind_rows(root_correlation_non_GC[[i]])
}
root_correlation_GC <- bind_rows(root_correlation_GC)
root_correlation_non_GC <- bind_rows(root_correlation_non_GC)
# Plotting methylation correlation
root_correlation_GC <- root_correlation_GC %>%
  mutate(time = gsub(".*_","",sample),sample = gsub("_.*","",sample))
root_correlation_GC$time <- factor(root_correlation_GC$time,levels = c("6h","12h","24h"))

root_correlation_non_GC <- root_correlation_non_GC %>%
  mutate(time = gsub(".*_","",sample),sample = gsub("_.*","",sample))
root_correlation_non_GC$time <- factor(root_correlation_non_GC$time,levels = c("6h","12h","24h"))

colours <- c('#8b0000', '#a56252', '#a9a9a9', '#8e65b1', '#00008b')
ggplot(root_correlation_GC,aes(x = position,y = correlation,colour = sample)) +
  geom_line() +
...
ggsave("~/iNOME/Figures/Supplementary/root_correlation_GC_Col0.pdf")

ggplot(root_correlation_non_GC,aes(x = position,y = correlation,colour = sample)) +
  geom_line() +
  facet_grid(cols = vars(context),rows = vars(time)) +
  scale_colour_manual(values = colours) +
  scale_x_continuous(labels = c(50,100,200,500,1000,2000,5000),breaks = seq(1,7,1)) +
  ...
  theme_cowplot() +
  theme(strip.background = element_blank(),axis.text.x = element_text(angle = 45,vjust = 0.5),axis.text = element_text(size = 8))
ggsave("~/iNOME/Figures/Supplementary/root_correlation_non_GC_Col0.pdf")
